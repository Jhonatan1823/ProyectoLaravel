...
class RolesController extends Controller
{
    ...
    public function store(Request $request){
        ...
        return redirect()->route('roles.index')->with('success','Rol con este codigo Registrado en la Plataforma');
    }

    //Udate
    public function update(Request $request, $Codigo_Rol){
        $request->validate([
            'Codigo_Rol' => 'required|unique:roles,Codigo_Rol,'. $Codigo_Rol . ',Codigo_Rol',
            'Descripcion_Rol' => 'required'
        ],[
            'Codigo_Rol.unique' => 'El rol con este con este codigo ya existe en la plataforma.',
        ]);
        $chat = RolesModelo::findOrFail($Codigo_Rol);
        $chat->update([
             'Codigo_Rol' => $request->Codigo_Rol,
             'Descripcion_Rol' => $request->Descripcion_Rol,
        ]);
           return redirect()->route('roles.index')->with('success','El rol ha sidi Actualizado en la Plataforma');

    }
    // Eliminar
public function destroy($idocumento)
{
    $chat = RolesModelo::findOrFail($idocumento);
    $chat->delete();

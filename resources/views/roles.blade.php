    ...
    <div class="card">
        <div class="card-body" style="width: 1200px;">
            ...
            <form action="{{ route('roles.index') }}" method="GET">
                <div class="text-end mb-3">
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" style="background-color:red;" data-bs-target="#AgregarModal"><i class="fa-solid fa-plus"></i> Nuevo Rol</button>
                </div>
                <div class="row g-2 align-items-center">
                    <div class="col-md-6">
                        <div class="input-group mb-3">
                            <span class="input-group-text"><i class="fas fa-search"></i></span>
                            <input type="text" class="form-control" name="search" value="{{ request('search') }}" placeholder="Buscar por código o descripción de rol" aria-label="Search" aria-describedby="basic-addon1">
                        </div>
                    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <button
                                    ...
                                    data-codigo="{{ $item->Codigo_Rol }}"
                                    data-descripcion="{{ $item->Descripcion_Rol }}">
                                ...
                                <form action="{{ route('roles.destroy', $item->Codigo_Rol) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger" style="background-color:black;color:white;" onclick="return confirm('¿Estás seguro de eliminar el rol con código: {{ $item->Codigo_Rol }}?')">
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-end">
                        <li class="page-item {{ $datos->onFirstPage() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $datos->previousPageUrl() }}{{ request('search') ? '&search=' . request('search') : '' }}">
                                Atras
                            </a>
                        </li>
                        ...
                        @for ($i = 1; $i <= $datos->lastPage(); $i++)
                            <li class="page-item {{ $datos->currentPage() == $i ? 'active' : '' }}">
                                <a class="page-link" href="{{ $datos->url($i) }}{{ request('search') ? '&search=' . request('search') : '' }}">
                                    {{ $i }}
                                </a>
                            </li>
                        @endfor
                        ...
                        <li class="page-item {{ !$datos->hasMorePages() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $datos->nextPageUrl() }}{{ request('search') ? '&search=' . request('search') : '' }}">
                                Siguiente
                            </a>
                        </li>
                    </ul>
            ...
            @else
                <div class="alert alert-info text-center mt-3">
                    ...
                    @if(request('search'))
                        No se encontraron roles con el criterio de búsqueda "{{ request('search') }}"
                    @else
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="AgregarModalLabel"><i class="fa-solid fa-user-plus"></i> Crear Nuevo Rol</h5>
                    ...
                    <div class="modal-body">
                        <form action="{{ route('roles.store') }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="Codigo_Rol" class="form-label">Código de Rol</label>
                                <input type="text" class="form-control" id="Codigo_Rol" name="Codigo_Rol" placeholder="Ej: ROL_ADM" required>
                                @error('Codigo_Rol')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="Descripcion_Rol" name="Descripcion_Rol" placeholder="Ej: Administrador del Sistema" required>
        ...
        <div class="modal fade" id="EditarModal" tabindex="-1" aria-labelledby="EditarModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="EditarModalLabel"><i class="fa-solid fa-pen-to-square"></i> Editar Rol</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editForm" method="POST">
                            ...
                            @method('PUT')
                            <div class="mb-3">
                                <label for="editCodigo_Rol" class="form-label">Código de Rol</label>
                            ...
                            <div class="mb-3">
                                <label for="editDescripcion_Rol" class="form-label">Descripción del Rol</label>
                                <input type="text" class="form-control" id="editDescripcion_Rol" name="Descripcion_Rol" required>
                                @error('Descripcion_Rol')
                                    <div class="text-danger mt-1">{{ $message }}</div>
                                @enderror
                            </div>
...
<script>
    ...
    editarModal.addEventListener('show.bs.modal', function (event) {
        ...
        // Llenar modal con los datos del rol
        document.getElementById('editCodigo_Rol').value = codigo;
        document.getElementById('editDescripcion_Rol').value = descripcion;

        // Cambiar la acción del formulario al endpoint de actualización
        var form = document.getElementById('editForm');
        // La ruta de actualización es 'roles/{Codigo_Rol}'
        form.action = '{{ url('roles') }}/' + codigo;
    });
</script>
